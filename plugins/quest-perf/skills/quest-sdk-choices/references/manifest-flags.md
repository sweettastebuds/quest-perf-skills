...
`aapt2` prints `android:name` and `android:value` on separate `A:` lines; the
`android:value` line follows each `android:name` line, so `-A1` keeps it.
...
| `<uses-feature android:glEsVersion="0x00030001">` | Unity 6.6 GLES floor | Emitted when OpenGLES3 is in the Android API list; ES 3.1 shader variants (when 'Use OpenGL ES 3.0 shaders' is off) raise MAX_VISIBLE_LIGHTS 16 to 32 (U1-094, G1-018) | GLES builds, Unity ≥ 6000.6 | gles3-perf:gles-versions-unity-output | https://docs.unity3d.com/6000.6/Documentation/Manual/UpgradeGuideUnity66.html (U1-094, G1-017, G1-018) | [doc] |
...
| Passthrough feature declaration | present / absent | An app that enables passthrough features (Meta's wording) loses CPU L4 and GPU L3-4 on Quest 3/3S; whether a manifest declaration alone triggers this is undocumented [verify on device]. The exact manifest key is not recorded in the dossier; inspect what the Meta SDK / OpenXR Camera (Passthrough) feature emits | Quest 3, Quest 3S (level loss); Quest 2 passthrough has no documented level effect | quest-perf:quest-mr-costs | https://developers.meta.com/horizon/documentation/unity/unity-passthrough-gs/ ; https://developers.meta.com/horizon/documentation/unity/os-cpu-gpu-levels/ (Q4-001, Q4-004, Q4-005) | [doc] [verify on device] |
